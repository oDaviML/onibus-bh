...
export default function Cardonibus({ linha, isFavorite, toggleFavorite }: CardonibusProps) {
	const allowedPaths = {
			...
			<DialogTrigger asChild>
				<Card className="hover:scale-105 hover:shadow-lg hover:cursor-pointer transition-all min-w-full sm:min-w-[300px]">
					<CardHeader>
						...
						<CardDescription className="font-light text-sm break-words">{linha.nome}</CardDescription>
					</CardHeader>
				</Card>
			</DialogTrigger>
			<DialogContent>
				...
				<div className="flex gap-4 flex-wrap justify-center">
					{linha.sentidoIsUnique ? (
						<Link to={allowedPaths.ida}>
							<Card className="w-[200px] h-[200px] flex items-center justify-center">
								{" "}
								<CardHeader>
